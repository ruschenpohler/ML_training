...
Given some set of predictors, we would like to predict an outcome: $\mathbb { E } \left[ y_{i} | x_{i} \right]$.

That is, what is $y_{i}$ given $x_{i}$?

- Example:
+ What impact does policy $x_{i}$ have on outcome $y_{i}$?
+ Does predictor $x_{i}$ influence behavior $y_{i}$?
...
First, we need to link the predictors $x_{i}$ to the outcome $y_{i}$:

$$\mathbb { E } \left[ y_{i} | x_{i} \right] = f \left( \eta _ { i } \right)$$

For simplicity, let $\eta_{i}$ be a linear fct of $x_{i}$:

$$\eta _ { i } = \alpha + x_{i} \beta$$

That is, we assume that a straight line is a good approximation for the impact of $x_{i}$ on $y_{i}.

But we still don't know **what straight line best approximates the relationship**!
...
That is, we consider the squared distance between our model $\eta_{i}$ and reality $y_{i}$.
...
library(png)
library(grid)
#imgpath <- system.file('pics/SGD.png',package='imager') 
#SGD <- load.image(imgpath)
SGD_2 <- readPNG("pics/SGD_2.png")
#SGD = read.gif("pics/SGD_2.gif", frame=0, flip=FALSE, verbose=FALSE)
